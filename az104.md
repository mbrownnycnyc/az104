...
- [Azure Administration](#azure-administration)
  - [Understanding Azure Resource Manager](#understanding-azure-resource-manager)
    ...
    - [summary Azure Resource Manager](#summary-azure-resource-manager)
  - [using azure portal and cloud shell](#using-azure-portal-and-cloud-shell)
    ...
    - [summary azure portal and cloud shell](#summary-azure-portal-and-cloud-shell)
  - [using Azure CLI and Powershell](#using-azure-cli-and-powershell)
    ...
    - [summary Azure CLI and Powershell](#summary-azure-cli-and-powershell)
  - [Using ARM templates](#using-arm-templates)
    ...
    - [summary ARM templates](#summary-arm-templates)
- [governance and compliance](#governance-and-compliance)
  - [Managing Subscriptions](#managing-subscriptions)
    ...
    - [summary Subscriptions](#summary-subscriptions)
  - [using management groups](#using-management-groups)
    ...
    - [demo:](#demo)
    - [summary using management groups](#summary-using-management-groups)
  - [Understanding Azure Policy](#understanding-azure-policy)
    ...
    - [summary Understanding Azure Policy](#summary-understanding-azure-policy)
  - [tagging resources](#tagging-resources)
    ...
    - [summary tagging resources](#summary-tagging-resources)
  ...
  - [Logging and moving resources](#logging-and-moving-resources)
    ...
    - [summary Logging and moving resources](#summary-logging-and-moving-resources)
  - [managing azure costs](#managing-azure-costs)
    ...
    - [summary managing azure costs](#summary-managing-azure-costs)
  - [building a cloud governance strategy with azure tool](#building-a-cloud-governance-strategy-with-azure-tool)
    ...
    - [summary building a cloud governance strategy with azure tool](#summary-building-a-cloud-governance-strategy-with-azure-tool)
    - [expand knowledge](#expand-knowledge)
- [Identity](#identity)
  - [summary Identity in Azure](#summary-identity-in-azure)
  - [conceptualizing Entra ID](#conceptualizing-entra-id)
    ...
    - [summary conceptualizing Entra ID](#summary-conceptualizing-entra-id)
  - [managing tenants](#managing-tenants)
    ...
    - [summary managing tenants](#summary-managing-tenants)
  - [creating and managing users](#creating-and-managing-users)
    ...
    - [summary creating and managing users](#summary-creating-and-managing-users)
  ...
  - [creating and managing groups](#creating-and-managing-groups)
    ...
    - [summary creating and managing groups](#summary-creating-and-managing-groups)
  ...
  - [Creating administrative units](#creating-administrative-units)
    ...
    - [summary Creating administrative units](#summary-creating-administrative-units)
  - [configure self-service password reset (SSPR)](#configure-self-service-password-reset-sspr)
    ...
    - [summary configure self-service password reset (SSPR)](#summary-configure-self-service-password-reset-sspr)
  - [Entra ID Device management](#entra-id-device-management)
    ...
    - [summary Entra ID Device management](#summary-entra-id-device-management)
...
- [Role-based access control (RBAC)](#role-based-access-control-rbac)
  - [understanding roles in Azure](#understanding-roles-in-azure)
    ...
    - [summary understanding roles in Azure](#summary-understanding-roles-in-azure)
  - [Assigning access to resources](#assigning-access-to-resources)
    ...
    - [summary Assigning access to resources](#summary-assigning-access-to-resources)
  ...
  - [Creating custom roles](#creating-custom-roles)
    ...
    - [summary Creating custom roles](#summary-creating-custom-roles)
- [Azure Storage](#azure-storage)
  - [Understanding Storage Accounts](#understanding-storage-accounts)
    ...
    - [summary Understanding Storage Accounts](#summary-understanding-storage-accounts)
  - [conceptualizing azure blob storage](#conceptualizing-azure-blob-storage)
    ...
    - [summary conceptualizing azure blob storage](#summary-conceptualizing-azure-blob-storage)
  - [configuring blob object replication](#configuring-blob-object-replication)
    ...
    - [summary configuring blob object replication](#summary-configuring-blob-object-replication)
  - [configuring blob lifecycle management](#configuring-blob-lifecycle-management)
    ...
    - [summary configuring blob lifecycle management](#summary-configuring-blob-lifecycle-management)
  ...
  - [configuring azure files](#configuring-azure-files)
    ...
    - [summary configuring azure files](#summary-configuring-azure-files)
  - [configuring azure File Sync](#configuring-azure-file-sync)
    ...
    - [summary configuring azure File Sync](#summary-configuring-azure-file-sync)
  ...
  - [storage network access](#storage-network-access)
    ...
    - [summary storage network access](#summary-storage-network-access)
  - [securing storage accounts](#securing-storage-accounts)
    ...
    - [summary securing storage accounts](#summary-securing-storage-accounts)
  ...
  - [Using Azure Jobs](#using-azure-jobs)
    ...
    - [summary Using Azure Jobs](#summary-using-azure-jobs)
  - [Storage utilties](#storage-utilties)
    ...
    - [demo: using storage utilities](#demo-using-storage-utilities)
      ...
      - [azcopy](#azcopy)
    - [summary Storage utilties (storage explorer vs. AzCopy)](#summary-storage-utilties-storage-explorer-vs-azcopy)
- [sanity check](#sanity-check)
  - [how burned out do I really want to be?](#how-burned-out-do-i-really-want-to-be)
  - [summary: how burned out do I really want to be?](#summary-how-burned-out-do-i-really-want-to-be)
- [Virtual networks](#virtual-networks)
  - [Conceptualizing Virtual Networks](#conceptualizing-virtual-networks)
    ...
    - [summary Conceptualizing Virtual Networks](#summary-conceptualizing-virtual-networks)
  - [creating virtual networks](#creating-virtual-networks)
    ...
    - [summary creating virtual networks](#summary-creating-virtual-networks)
  ...
  - [deploying network resources](#deploying-network-resources)
    ...
    - [summary deploying network resources](#summary-deploying-network-resources)
  - [routing VNets](#routing-vnets)
    ...
    - [summary routing VNets](#summary-routing-vnets)
  - [Network Security Groups](#network-security-groups)
    ...
    - [summary Network Security Groups](#summary-network-security-groups)
  ...
  - [Using Azure DNS](#using-azure-dns)
    ...
    - [summary Using Azure DNS](#summary-using-azure-dns)
  - [using azure firewall](#using-azure-firewall)
    ...
    - [summary using azure firewall](#summary-using-azure-firewall)
  ...
  - [using service endpoints](#using-service-endpoints)
    ...
    - [summary using service endpoints](#summary-using-service-endpoints)
  - [Using Private Endpoints](#using-private-endpoints)
    ...
    - [summary Using Private Endpoints](#summary-using-private-endpoints)
- [intersite connectivity](#intersite-connectivity)
  - [configuring azure VNet peering](#configuring-azure-vnet-peering)
    ...
    - [summary configuring azure VNet peering](#summary-configuring-azure-vnet-peering)
  ...
  - [demo: implementing VPN Gateway](#demo-implementing-vpn-gateway)
    - [summary Implementing VPNs](#summary-implementing-vpns)
  - [Configure ExressRoute](#configure-exressroute)
    ...
    - [summary Configure ExressRoute](#summary-configure-exressroute)
  - [implementing Virtual WAN](#implementing-virtual-wan)
    ...
    - [summary implementing Virtual WAN](#summary-implementing-virtual-wan)
...
- [Azure VMs](#azure-vms)
  - [creating and managing VMs](#creating-and-managing-vms)
    ...
    - [summary creating and managing VMs](#summary-creating-and-managing-vms)
  - [Managing VM disks](#managing-vm-disks)
    ...
    - [summary Managing VM disks](#summary-managing-vm-disks)
  ...
  - [configuring VM scale sets](#configuring-vm-scale-sets)
    ...
    - [summary configuring VM scale sets](#summary-configuring-vm-scale-sets)
  ...
  - [Automating VM deployments](#automating-vm-deployments)
    ...
    - [summary Automating VM deployments](#summary-automating-vm-deployments)
  ...
  - [Managing Virtual Machine Updates](#managing-virtual-machine-updates)
    ...
    - [summary Managing Virtual Machine Updates](#summary-managing-virtual-machine-updates)
  - [Automating VM configs](#automating-vm-configs)
    ...
    - [summary Automating VM configs](#summary-automating-vm-configs)
  - [Using Azure Bastion](#using-azure-bastion)
    ...
    - [summary Using Azure Bastion](#summary-using-azure-bastion)
...
- [network traffic mgmt](#network-traffic-mgmt)
  - [Introducing Azure Load Balancer](#introducing-azure-load-balancer)
    ...
    - [summary Introducing Azure Load Balancer](#summary-introducing-azure-load-balancer)
  ...
  - [Using Application Gateway](#using-application-gateway)
    ...
    - [summary Using Application Gateway](#summary-using-application-gateway)
- [web apps and containers](#web-apps-and-containers)
  - [creating an app service plan](#creating-an-app-service-plan)
    ...
    - [app service plan compute types:](#app-service-plan-compute-types)
    - [traditional vs. app service](#traditional-vs-app-service)
    - [demo: create app service plan](#demo-create-app-service-plan)
    - [summary creating an app service plan](#summary-creating-an-app-service-plan)
  - [creating web apps](#creating-web-apps)
    - [web app service features](#web-app-service-features)
    - [web app architecture](#web-app-architecture)
    - [demo: creating web app](#demo-creating-web-app)
    - [summary creating web apps](#summary-creating-web-apps)
  - [configuring web apps](#configuring-web-apps)
    - [configuration options](#configuration-options)
      - [web app custom domain](#web-app-custom-domain)
      - [web app scaling](#web-app-scaling)
      - [web app deployment slots](#web-app-deployment-slots)
      - [web app network settings](#web-app-network-settings)
      - [web app backup](#web-app-backup)
    - [demo: review Web App options](#demo-review-web-app-options)
    - [summary configuring web apps](#summary-configuring-web-apps)
  - [describing containers in Azure](#describing-containers-in-azure)
    - [what are containers](#what-are-containers)
    - [container development concepts](#container-development-concepts)
    - [demo: containers](#demo-containers)
    - [summary describing containers in Azure](#summary-describing-containers-in-azure)
  - [using Azure Container Instances for container hosting](#using-azure-container-instances-for-container-hosting)
    - [describing Azure Container inages](#describing-azure-container-inages)
    - [demo:](#demo-1)
  - [LAB: Build and Run a Container Using Azure ACR Tasks](#lab-build-and-run-a-container-using-azure-acr-tasks)
  - [LAB: Create Web App from Docker Container in Azure](#lab-create-web-app-from-docker-container-in-azure)
  - [Using Azure Kubernetes Service (AKS)](#using-azure-kubernetes-service-aks)
    - [traditional Azure Container Instance solution](#traditional-azure-container-instance-solution)
    - [Kubernetes container solution](#kubernetes-container-solution)
    - [what is AKS?](#what-is-aks)
    - [AKS features:](#aks-features)
    - [AKS Architecture](#aks-architecture)
    - [demo: create AKS clsuter](#demo-create-aks-clsuter)
- [monitoring](#monitoring)
  - [Using Azure Monitor](#using-azure-monitor)
    - [describing azure monitor](#describing-azure-monitor)
    - [metrics vs. logs](#metrics-vs-logs)
    - [monitoring explained](#monitoring-explained)
    - [demo: interpreting metrics (isong Metrics explorer)](#demo-interpreting-metrics-isong-metrics-explorer)
    - [summary Using Azure Monitor](#summary-using-azure-monitor)
  - [setting up alerting and actions](#setting-up-alerting-and-actions)
    - [describing azure monitor alerts](#describing-azure-monitor-alerts)
    - [action groups](#action-groups)
      - [action types:](#action-types)
    - [demo: creating an alert](#demo-creating-an-alert)
    - [summary setting up alerting and actions](#summary-setting-up-alerting-and-actions)
  - [configure Azure Monitor logs](#configure-azure-monitor-logs)
    - [describing Azure Monitor Logs (aka Log Analytics)](#describing-azure-monitor-logs-aka-log-analytics)
    - [Components of Log Analytics](#components-of-log-analytics)
    - [demo: configure log analytics](#demo-configure-log-analytics)
    - [summary configure Azure Monitor logs](#summary-configure-azure-monitor-logs)
  - [Understanding Monitor Insights](#understanding-monitor-insights)
    - [describing Insights](#describing-insights)
    - [VM and Network Insights review](#vm-and-network-insights-review)
    - [container and application Insights](#container-and-application-insights)
    - [demo: review Insights](#demo-review-insights)
    - [summary Understanding Monitor Insights](#summary-understanding-monitor-insights)
  - [Configuring Application Insights](#configuring-application-insights)
    - [Describing Application Insights](#describing-application-insights)
    - [Application Insight features](#application-insight-features)
    - [architectural Components:](#architectural-components)
    - [demo: configure App Insights](#demo-configure-app-insights)
    - [summary Configuring Application Insights](#summary-configuring-application-insights)
  - [using Network Watcher](#using-network-watcher)
    - [describing Azure Network Watcher](#describing-azure-network-watcher)
    - [demo: Using Network Watcher](#demo-using-network-watcher)
    - [summary using Network Watcher](#summary-using-network-watcher)
- [Backup and recovery](#backup-and-recovery)
  - [Understanding Disaster Recovery](#understanding-disaster-recovery)
    - [describing disaster recovery](#describing-disaster-recovery)
    - [Recovery point objective (RPO) vs. recovery time object (RTO)](#recovery-point-objective-rpo-vs-recovery-time-object-rto)
    - [summary Understanding Disaster Recovery](#summary-understanding-disaster-recovery)
  - [Configuring Azure Backup](#configuring-azure-backup)
    - [describing Azure Backup](#describing-azure-backup)
    - [Components of Azure Backup](#components-of-azure-backup)
    - [demo: using Azure Backup](#demo-using-azure-backup)
    - [summary Configuring Azure Backup](#summary-configuring-azure-backup)
  - [LAB: restore file share from Azure Backup to another storage account](#lab-restore-file-share-from-azure-backup-to-another-storage-account)
  - [Azure Site Recovery](#azure-site-recovery)
    - [Describing Azure Site Recovery](#describing-azure-site-recovery)
    - [Components of Azure Site Recovery](#components-of-azure-site-recovery)
    - [demo: config Site-to-Site recovery](#demo-config-site-to-site-recovery)
    - [summary Azure Site Recovery](#summary-azure-site-recovery)
  - [backup reports](#backup-reports)
    - [describe backup reports](#describe-backup-reports)
    - [components of backup reports](#components-of-backup-reports)
    - [demo: config backup reports](#demo-config-backup-reports)
    - [summary backup reports](#summary-backup-reports)
...
## summary: how burned out do I really want to be?
* it doesn't matter.
...
* defines a set of compute resources for a web app to run on
  ...
  * number of VMs: the number of VM instances running under a plan
  * size of VMs: the size of VM instances (small, medium, large)
...
* from least expensive to most expensive:
  * Share: run apps on the same VM as other apps, includign apps of other customers (cannot scale out because of shared compute)
  * dedicated: run only apps using the same App Service Plan on a dedicated VM (isolated compute)
  * Isolated: run apps using dedicated VMs and dedicate VNet (isolated compute and network)

### traditional vs. app service
* traditional
  * data center
  * in server racks
  * compute resourecs inside racks
  * LB for balancing access
  * admin and manage hardware
  * manage patching and security
  * managing LB
* app service plan
  * app service determines the distribution and serving of the app code
  * VM compute resources are underlying the service
  * managed infra
  * HA
  * scalability (with supported pricing tiers for isolated)

### demo: create app service plan
* login to ARM> search> app service plan> create
  ![](2024-08-08-20-45-12.png)
  ![](2024-08-08-20-45-21.png)
* review settings
  ![](2024-08-08-20-46-22.png)
* you can scale up to a higher plan
* you can scale out by increasing up to three instances
...
* managed infrastructure: no need to patch, maintain, implement or config underlying infra components with this PaaS
* highly available: App Service runs apps on multiple nodes to provide HA
* autoscaling in/out: scaling capabiliteis similar to scale sets ensure you can meet the traffic demands of your app
* Development focused: dev of app code is the focus for web app service deployments
* deployment slots: CI/CD DevOps provides features such as staging slots for eeployments
* azure service integration: App Service integrates several Azure services, such as Entra ID as an IdP or VNets for connecting App SErvice to resources within a virtual network

### web app architecture
* App Service Plan: deploy a web app
* underlying compute provided
* DB can be offered
* deploy to the App Service Plan using

### demo: creating web app
* login to ARM> app services> create
  ![](2024-08-08-20-52-55.png)
  * publish: code or docker container
  * runtime stack: the code base
  * create or select an existing App Service Plan
    ![](2024-08-08-20-53-56.png)
  * can deploy with github actions integration
    ![](2024-08-08-20-54-32.png)
  * App Insights
    ![](2024-08-08-20-54-47.png)
* open the cloud shell
* create a .net app with `dotnet` CLI
* publish the .net app with `dotnet` CLI
* deploy the published app
  * zip the published app directory
  ![](2024-08-09-09-37-36.png)
...
* custom domain provide a customer domaint o be used by the web app
* scaling: specify scaling options to scale up/out compute resources for your web apps
* deployment slots: CI/CD DevOps provides features such as staging slots for eeployments
* network: manage network settings and integration by doing things such as connecting web apps with VNets, CDN, etc
* backup: backup web apps using full architval backups or incremental snapshots that are stored in a storage account via the Blob Storage Service

#### web app custom domain
* the domain name provided to webapps is automatically assigned as `[app name].azurewebsites.net`
  * to verify ownership and use a custom domain name, you can provide a verification code via a TXT record, then Azure will use the custom domain name.  You will also need to create an A record

#### web app scaling
* scale out/back
* scale up/down: upgrading app service plan

#### web app deployment slots
* app can be running production slot, and staging slot... no extra money
  * swap slots between staging and prod to push the app

#### web app network settings
* if using Isolated, can deploy to VNet itself
* web apps can integrate into a VNet if no Isolated
  * this provides access to resources within VNets to the web app

#### web app backup
* create a blob in a storage account
* can be stored as full backup or snapshot
* items backed up
  * app config
  * file content
  * database connection

### demo: review Web App options
* go to App Service instance
  * review deployment slots
    ![](2024-08-09-09-53-46.png)
  * review deployment Center
    ![](2024-08-09-09-54-04.png)
  * can provide connection strings and env vars
    ![](2024-08-09-09-54-26.png)
  * can provide IdP integration
    ![](2024-08-09-09-54-44.png)
  * can implement backups
    ![](2024-08-09-09-55-05.png)
  * integrate into a custom domain
    ![](2024-08-09-09-56-14.png)
    ![](2024-08-09-09-56-24.png)
  * TLS/SSL settings
    ![](2024-08-09-09-56-39.png)
  * Networking configs
   ![](2024-08-09-09-56-54.png)
   * this includes access restrictions
  * scaling
...
### what are containers
* a container is a unit of software.  This unit consists of code, its dependencies and configs to be shipped and run anywhere
  * container: an instance of a container image runnign
  * container image: build an instance of a container, a template of a container built from a Docker file
  * container registry: Azure Container Registry can be used to store container images
  * container host: Azure Container Instances run the container images
  * DockerFile: a definition of all code and dependencies in a container solution.

### container development concepts
* write code, create Container, push

### demo: containers
* creating containers follow a similar process regardless of where they are hosted
* create a container registry: create the Azure Container Registry that will act as the repo
* Push container to Registry: push the container image to the Azure Container Registry, which is a repo of container images
...
## using Azure Container Instances for container hosting

### describing Azure Container inages
* ACI is a container hosting service much like Docker
* defining container groups: a collection of containers that share a lifecycle, resources, local networking and storage
* container storage: can use Azure file shares for persistent data storage (Azure Files)
* container network: can deploy containers as either private or public resources (can deploy to VNet and can attach a Public IP and DNS name)

### demo: 
* create a container group using ACR (azure container registry)
* update Container Group: update the container group with an FQDN
  
* login ARM> container instances> Add
  * selecting image source and size
    ![](2024-08-09-10-22-50.png) 
  * networking: choose public or private
    ![](2024-08-09-10-24-24.png)
  * advanced: restart policy, env vars
    ![](2024-08-09-10-25-17.png)
* review:
  ![](2024-08-09-10-26-18.png)
* review settings:
  * Containers:
    ![](2024-08-09-10-27-06.png)
  * identities


* deploy a multi-container solutoin: using a HSON file to create a multi-container container group
* you can use an ARM template to create azure container group
  ![](2024-08-09-10-28-47.png)

## LAB: Build and Run a Container Using Azure ACR Tasks
* Start Cloud Shell
* Create a New Container Registry
```
az acr create --resource-group "425-a6b38fe1-build-and-run-a-container-using-azure" --name acrbuildcontainer11 --sku Basic --admin-enabled true
...
## LAB: Create Web App from Docker Container in Azure
...
* Create and Deploy a New Web App
  * create a Web App via App Services in ARM, using Standard S1 size, and source the image from ACR

## Using Azure Kubernetes Service (AKS)

### traditional Azure Container Instance solution
* group of containers may be used... but you must manage them specifically (scaling out manually as necessary)
* Limitations of ACI:
...
* Managmenet: provides an orchestration layer for clusters

### what is AKS?
![](2024-08-09-10-40-01.png)
* What?
  * AKS is a serverless platform for kubernetes.  It provides scalable container solutions.
* Why?
  * AKS abstracts away the mgmt of a kubernetes cluster by managing most of the deployment and orchestration

### AKS features:
* management: autoscalting of clsuters and horizontal scaling of pods.  Manage node pool configs and kubernetes versioning and upgrades
* Identity: provide access to AKS cluster using Entra ID identity objects and RBAC. Provide an identity for the cluster to manage underlying resources.
* Networking: VNet conncetivity integrated via kubenet or Container Network Interface (CNI).  HTTP app routing avaialble.  Manage trafic using network policies
* Storage: implement storage voluems that are either static or dynamic.
* Registry: store contaienr images in Azure Container registry, Docker Hub or other public/private repos.  Docker images are supported.

### AKS Architecture
* within a VNet
  * "kubenet" provides networking access into the node pools
  * CNI per container
* AKS cluster has:
  * a management node
  * worker nodes
...
### demo: create AKS clsuter
* login to ARM> Kubernetes Services> Create
  ![](2024-08-09-10-46-02.png)
  * node size and node count
    ![](2024-08-09-10-46-18.png)
  * node pools can be added and distributed across AZs
    * ACI can provide burstable scaling
    * You can also use a VM Scale Set (which is required for multi-AZ deployments)
    ![](2024-08-09-10-47-13.png)
  * authentication
    * Cluster infra: managing the clsuter
    * kubernetes auth (namespaces)
      ![](2024-08-09-10-56-38.png)
  * networking:
    * kubenet: nodes have IPs
    * Azure CNI: new or existing VNet giving pods IP (overlay network)
      ![](2024-08-09-10-57-24.png)
    * can restirct IP ranges
  * integrations:
    * Azure Registry, Azure Monitor and Azure policy integrations
      ![](2024-08-09-10-58-14.png)
* scaling is possible:
  ![](2024-08-09-10-58-37.png)
...
* Full stack monitoring: provides end-to-end monitoring for apps and underlying infra
...
* Azure has metrics and logs
* scope them and view them
  * Azure Monitor: rcvs metrics only
    * view with Metrics Epxlorer
  * Log Analytics: rcvs metrics and logs
    * query Log Analytics
  * Storage Accounts: rcvs metrics and logs
  * Events Hub: rcvs metrics and logs
    * can pass to destinations outside of Azure

### demo: interpreting metrics (isong Metrics explorer)
* login to ARM> go to VM, Monitoring/Metrics
  * brief UI is available here
* search resources search bar and type "monitor" and go to Metrics Explorer
  * can select scope and select metric
* in monitor, you can go to logs and go to the Log Analytics UI
  * you can attach logging policies to each resoruce via Settings/Diagnostics, and then send to a workspace
    ![](2024-08-09-11-14-21.png)
...
## setting up alerting and actions

### describing azure monitor alerts
Azure Monitor Alerting...
* alerts on signals that prompt you to take proactive actiosn and help automate monitoring and diagnostics
* signal types: metric, activity and log signals
* action group: actions that will take place when an alert has been triggered

### action groups
* capturing metircs and logs
* then create an alert rule to scope resource and condition
  * action groups are a result of this
...
### demo: creating an alert
* access ARM> access a VM
* go to resource search, montiro> alerts> create new alert role
* select a VM resource
* set the condition on (signal types ), deallocate VM
* set an action, then create an action group
  * notifications
  * actions: action type: automation runbook -> restart if stopped
* create the alert
...
### describing Azure Monitor Logs (aka Log Analytics)
* Log Analytics: a service for aggregating log data in a signle pna,e where it can be analyzed, virtualized, and queried
* sources of log data include:
...
* log analytics workspace
  * access to repo for data
* data inbound:
  * data is streamed and stored in a Workspace (repo)
* Visualize, alert, and provide insights

### demo: configure log analytics
* login to ARM> observe two VMs
* go to resource search bar> Log Analytics workspaces> create
* inside of the resource, add some data sources (workspace data sources/virtual machines), can easily connect Azure resources
  ![](2024-08-09-11-31-00.png)
* then access General/Logs on the workspace itself
  * and there are canned queries available
...
### describing Insights
* service monitoring: service-specific monitoring features built into Azure Monitors
  * VM Insights
  * Network Insights
  * Container Insights
  * Application Insights

### VM and Network Insights review

* VM insights: monitoring service specific to virtual machiens and virtual machien scale sets (VMSS)
  * requires Log Analytics workspace
  * requires Log Analytics agent (installed when connected)
  * also known as "Azure Monitor for VMs"
* Network Insights: monitoring service specific to VNet resources
...
* Container Insights: monitor serviec specific to containers or AKS clusters
  * requires Log Analytics workspace
  * requires Log Analytics agent
* Application Insights: monitoring service specific to app code
  * App Insights resource
  * instrumentation of app code

### demo: review Insights
* login to ARM> create some resources that are in scope of Insights (VM, networks)
* access resource search bar> Monitor> review Insights> VMs
  * select Not Monitored
* go to Networks
  * this will automatically be enabled???
...
* Application Insights: a full stack application monitoring solution that devs can use to monitor apps.
  * supports any app instrumented with App Insights
  * repo for events and metrics data
  * telemetry data is streamingo an Application Insight resource (ie: page visits)

### Application Insight features
* Metrics: live Metrics Stream for new-real-time metrics data and Metrics Explorer for viewing how metrics vary over time
* alerts: alerting on metrics or event data to notify app admins of issues
* Profiler: determine how requests are delivered, such as page elements and their performance
* Application Map: a topological view of applications and dependencies, used to identify dependency issues such as bottlencks
* Usage Analytics: analyze user metrics from client-side events like user interaction

### architectural Components:
* collect data and stream to Application Insights
  * client-side: ddtermine client side insights
  * application: instrumentation needed
  * dependencies: instrumentation dependencies of the app

### demo: configure App Insights
* App Services> create > within monitoring enable Application Insights
* access to App Services Application Insights
  ![](2024-08-09-11-49-02.png)
* you need to have Devs integrate App Insights to aggregate app data
* review: Investigate/Application map (topological representation of dependencies)
  ![](2024-08-09-11-49-50.png)
* Live metrics of performance of apps, availability, failures
* Monitoring (alerts, metrics, diag, logs, log analytics workbooks) are avialble
* there is a requirement for when devs want to integrate logging into the app itself, that you can grab the Instrumentation key or connection string from the application Insights overview page:
  ![](2024-08-09-11-52-05.png)
  * can implement as an env var
    ![](2024-08-09-11-52-40.png)
...
* azure Network Watcher is a service comprised of networking tools for monitoring and diagnostics
  ...
  * tshoot connectivity issues
  * tshoot hybrid network solutions
  * eanble per region in subscription

### demo: Using Network Watcher
* create a vnet and app insights and app service

![](2024-08-09-11-59-01.png)

* resource search bar> Network Watcher> overview can enable/disable network watcher
  * Monitoring/topology: can see the VNet topology (including resources)
  * Monitoring/Connection monitor: creates a log analytics workgroup to review VNet to VNet flows (down to resource flows)
  * Monitoring/NEtwork Performance Monitor: will replace Connection Monitor
* Network diagnostic tools:
  * NSGs and effective security groups
...
* the process of recovering from a disaster, such as a data cente rpwoer outage.  Every business needs a BCDR plan
  * assess risks
  * determine critical workloads
  * decide backup technique
  * test DR

### Recovery point objective (RPO) vs. recovery time object (RTO)

![](2024-08-09-12-30-05.png)
* RPO: tolerance of data loss before recovery snapshot (aka backup) is taken
* RTO: how quickly the backup can be restored
...
* backup as a service: azure Backup is a managed service for backing up and recovering workloads for both Azure and on prem
* requires an Azure Recovery Service vault
* supported workloads:
  * VMs
  * on prem workloads
    ![](2024-08-09-12-45-45.png)
  * SQL server workloads
  * SAP HANA workloads

### Components of Azure Backup
* workload (in Azure or on prem)
* create a Recovery Services Vault
* within Vault, create an Azure Backup Backup Policy
* the Backup Policy contains a scheduled
* Recovery Services Vault is data repo for backups

### demo: using Azure Backup
* login to ARM> note VMs
  * for VMs, the backup is visible:
    ![](2024-08-09-12-42-36.png)
* create a new vault 
* create a policy
  * instant recovery snapshots
* restore point types:
  * crash consistent
  * application consistent
  * file-system consistent
* access the Recovery Service Vault and review the backup configuration:
  ![](2024-08-09-12-44-41.png)
  * note that the storage repl type modification is possible
  * the Service Vault allows for policy and backup reviews
* when restoring a VM, you must power down VM (for crash comsistent and file-system consistent)... you must target a Storage Account in order to not overwrite the existing data:
  ![](2024-08-09-12-49-20.png)
...
## LAB: restore file share from Azure Backup to another storage account
* yarp

## Azure Site Recovery

### Describing Azure Site Recovery
* provides a solution for automating DR
* Requires an Azure Recovery Service Vault
  * cross zone
  * cross region

### Components of Azure Site Recovery
* first physical location, with VMs that use managed disks
* Azure Site Recovery caches data into a storage account which is repled into a target Storage account in another region

### demo: config Site-to-Site recovery
* login to ARM, verify VMs, and the Recovery Services Vault
* In the Vault, enable replication
  * Use Resource Manager deployment
  * add the VMs in scope
  * Configure the source RG and Storage Account
  * Replication target selection, will automate creating all resources needed within the target region
* Access target RG and access the Recovery Services vault and review protected items/replicated items
  ![](2024-08-09-13-01-29.png)
  * you can performance various actions on the target replicated items
    ![](2024-08-09-13-01-56.png)
* you can create a Recovery Plan to coordinate many Replicated Items
...
### describe backup reports
* provide insight n backups from Azure Backup, and thsoe insights can be used to inform items such as:
  * forecasts for cloud storage consumption
  * audits of backup and restore events (for optimization)
* must use log analytics workspace as its logging service (Recovery SErvices Vault as source)

### components of backup reports
* Azure Backup enabled
* Log Analytics Workspace created
* Recovery Services Vault configured to forward data ot Log Analytics Workspace

### demo: config backup reports
* login ARM> create a Log Analytics Workspace
* access Recovery Services Vault
  * go to monitoring/diagnostic settings/ select the log data to capture
    * the categories needed for Backup Reports are:
      ![](2024-08-09-13-07-55.png)
    * send to Log Analytics workspace
      * destination table: resource specific
* you can then access Backup Reports within the Recovery Services Vault
  ![](2024-08-09-13-09-23.png)
...
* unlike AZ400, this exam was much more difficult as the content requires you to know much more detail about the services.  I recommend going through scenario labs and leveraging the web UI, and azcli and powershell to complete each lab.
* There are a lot of order of execution questions and scenario based questions that stack five pieces of data (some of which may be useless), which can be confusing to me.
  * To handle these questions, and possibly save time, what I do is the following:
    * Read the question first (the last sentence)
    * Record the sentence before that to gain more context
    * Then read the entire question with all details.
* prep videos:
...
* practice exams: do NOT use brain dumps as they can be outdated and have errors.
  * https://practice-exam.acloud.guru/a9fbf37a-41af-4632-a404-179ef6b5991e
...
* the AZ104 cert lasts a year, you can re-cert easily in a non-proctored, open book, free online test.
